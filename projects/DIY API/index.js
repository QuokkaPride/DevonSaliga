...
app.get("/jokes/:id", (req, res) => {
  ...
  // const id = jokes.find((jokes) => jokes.id ===id);
  res.json(jokes[id-1]);
});
...
app.post("/jokes", (req, res) => {
  ...
  data.id = id; // Add the id to the data object
  jokes.push(data); // Add the new joke to the jokes array
  console.log(jokes.slice(-1));
  res.json(data); // Send a response with the new joke
});
...
app.put("/jokes/:id", (req, res) => {
 ...
 const putJokeIndex = jokes.find((joke) => joke.id === id);
 jokes[putJokeIndex] = updatedJoke

res.json(jokes[putJokeIndex]);
})
//6. PATCH a joke
